...
class Rekapitulasi_model extends CI_Model
{
    ...
    public function getSebaranTpsPanakkukang()
    {
    ...
    public function getSebaranTpsBiringkanaya()
    {
        $query = "select tps, 
        ...
        FROM(SELECT namakel,namakec, tps, jml_suara FROM `rekap_suara` RIGHT join tbl_tps on rekap_suara.id_tps=tbl_tps.id_tps WHERE no_urut_calon= '01') 
        as dummy_table WHERE namakec ='BIRINGKANAYA' group by dummy_table.tps ORDER BY `dummy_table`.`tps`+0 asc";

        return  $this->db->query($query)->result_array();
    }

    public function getSebaranTpsManggala()
    {
        $query = "select tps, 
    ...
    public function getSebaranTpsTamalanrea()
    {
        $query = "select tps, 
